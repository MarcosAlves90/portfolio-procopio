...
export default function Strip() {
  ...
  const calculateMarqueeConfig = useCallback(
    (containerWidth: number, groupWidth: number): MarqueeConfig | null => {
      if (groupWidth === 0) return null;

      const repeatsNeeded = Math.max(
        1,
        Math.ceil(containerWidth / groupWidth) + 1,
      );
  ...
  const shouldUpdateConfig = useCallback(
    (prev: MarqueeConfig, next: MarqueeConfig): boolean => {
      return (
  ...
  useLayoutEffect(() => {
    ...
    const updateConfig = () => {
      ...
      const containerWidth = Math.round(
        currentContainer.getBoundingClientRect().width,
      );
      ...
      setConfig((prev) =>
        shouldUpdateConfig(prev, newConfig) ? newConfig : prev,
      );
      ...
      >
        ...
        >
          <div ref={measureRef} className="strip-group flex items-center">
            {BASE_ITEMS.map((text, i) => (
              <StripItem key={`m-${i}`} text={text} keyPrefix="m" index={i} />
            ))}
          </div>
        </div>
